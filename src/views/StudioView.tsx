...
export const StudioView: React.FC<StudioViewProps> = ({ character, setCharacter }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col pt-36 pb-64 animate-in fade-in duration-1000">
      ...
      {isChatOpen && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-8 bg-black/60 backdrop-blur-md animate-in fade-in duration-500">
           <div className="relative w-full max-w-5xl h-[85vh] art-glass rounded-[4rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] border-rose-900/40">
              <button 
                onClick={() => setIsChatOpen(false)}
                className="absolute top-8 right-8 z-[310] p-4 rounded-full bg-black/40 text-rose-500 hover:bg-rose-900/40 transition-all border border-rose-900/20"
              >
                <X className="w-6 h-6" />
              </button>
          ...
          <GlassCard padding="md" className="rounded-[2.5rem] flex items-center justify-between border-rose-900/40 hover-animate">
            ...
            <div className="flex gap-2">
              <button 
                ...
                title="Chat Now"
                className="p-4 bg-rose-600 text-white rounded-2xl hover:bg-rose-500 transition-all active:scale-90 shadow-lg"
              >
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[200] flex flex-col items-center gap-4">
         ...
         <button onClick={() => generate(promptInput)} disabled={isGenerating || !promptInput.trim()} className="w-64 h-24 bg-rose-800 text-white rounded-full font-black text-[14px] uppercase tracking-[0.4em] flex items-center justify-center gap-4 hover:bg-rose-700 shadow-2xl disabled:opacity-5 border-2 border-rose-950/40 group/breathe">
           {isGenerating ? <RefreshCw className="animate-spin w-8 h-8" /> : (
             <div className="flex items-center gap-4">
               <Zap className="w-8 h-8 group-hover/breathe:animate-icon-glow" />
               <MorphingText language={language} value="breatheLife" english="Breathe Life" className="serif-display italic font-thin text-3xl" />
             </div>
           )}
         </button>
      </div>
